...
def style_header_cell(cell, text, bold=True, size=12):
    """Style header cells - RTB official format"""
    cell.text = text
    ...
    run.font.name = 'Times New Roman'  # RTB uses Times New Roman
...
def set_cell_text(cell, text, bold=False):
    """Set cell text with Times New Roman 12"""
...
def generate_apa_references(topic, sector, learning_outcomes):
    """Generate real APA format references based on topic and sector"""
    topic_lower = topic.lower()
    sector_lower = sector.lower() if sector else ''
    
    refs = []
    
    # ICT/Software references
    ...
    # Hospitality/Food references
    elif 'hospitality' in sector_lower or 'food' in sector_lower or 'culinary' in topic_lower:
        refs.extend([
    ...
    # Construction/Engineering references
    elif 'construction' in sector_lower or 'engineering' in sector_lower:
        refs.extend([
    ...
    return "\n".join(refs[:4])  # Return top 4 most relevant

def generate_smart_objectives(topic, learning_outcomes, session_range=None, indicative_contents=None):
    """Generate RTB-formatted SMART objectives"""
    import re
    
    topic = (topic or 'the topic').strip()
    learning_outcomes = (learning_outcomes or '').strip()
    indicative_contents = (indicative_contents or '').strip()
    
    # Extract action verbs from learning outcomes
    lo_lower = learning_outcomes.lower()
    
    # Determine Bloom's taxonomy level and verbs
    if any(word in lo_lower for word in ['analyze', 'evaluate', 'create', 'design', 'develop']):
        verbs = ['Analyze', 'Evaluate', 'Create']
    elif any(word in lo_lower for word in ['apply', 'demonstrate', 'implement', 'solve']):
        verbs = ['Demonstrate', 'Apply', 'Implement']
    elif any(word in lo_lower for word in ['explain', 'describe', 'discuss', 'interpret']):
        verbs = ['Explain', 'Describe', 'Discuss']
    else:
        verbs = ['Define', 'Identify', 'List']
    
    # Extract key concepts from topic and content
    topic_words = [w for w in topic.split() if len(w) > 3]
    key_concept = ' '.join(topic_words[-2:]) if len(topic_words) >= 2 else topic
    
    # Build SMART objectives (Specific, Measurable, Achievable, Relevant, Time-bound)
    objectives = [
        f"{verbs[0]} {key_concept} and its application in {learning_outcomes[:50].lower()}.",
        f"{verbs[1]} at least 3 key concepts related to {topic.lower()} through practical exercises.",
        f"{verbs[2]} the knowledge gained to solve real-world problems in {key_concept.lower()}."
    ]
...
def generate_session_plan_docx(session_plan):
    ...
    # Set to LANDSCAPE orientation (matching RTB official)
    ...
    # Create the main table with 8 columns and 22 rows (EXACT RTB FORMAT)
    ...
    # Row 7: SMART Objectives (RTB Format)
    ...
    para = cell.paragraphs[0]
    run_bold = para.add_run("Objectives: ")
    run_bold.font.bold = True
    run_bold.font.size = Pt(12)
    run_bold.font.name = 'Bookman Old Style'
    run_subtitle = para.add_run("By the end of this session, every learner should be able to:\n")
    run_subtitle.font.bold = False
    run_subtitle.font.italic = True
    run_subtitle.font.size = Pt(11)
    run_subtitle.font.name = 'Bookman Old Style'
...
• Expresses readiness for next session'''
    ...
    # Row 19: References with APA format
    auto_references = generate_apa_references(
        session_plan.topic_of_session or 'the topic',
        session_plan.sector or '',
        session_plan.learning_outcomes or ''
    )
    references_text = session_plan.references or auto_references
    ...
    run_bold.font.size = Pt(12)
    run_bold.font.name = 'Bookman Old Style'
    run_normal = para.add_run(references_text)
